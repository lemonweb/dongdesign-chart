...
颜色审查必须区分“当前模式显示值”和“规范变量来源”。如果图表只写 resolved hex，却没有说明或保留 dongDesign Charts 全局变量族依赖，应判为 P1；如果该色值与全局变量体系明显冲突，应判为 P0 或 P1，取决于是否影响数据语义。

漏斗图额外审查项：

| 维度 | 检查内容 | 典型问题 |
|---|---|---|
| 漏斗变量依赖 | 阶段色是否依赖 `funnelPalette/*` 或同色相 `有序色板/*`，文本/边框/趋势是否依赖全局变量族 | 手写蓝色色值、转化率数值用阶段蓝、趋势变化用分类色 |
| 转化线形态 | 右侧相邻转化线是否为约 `116.81 x 77.14` 浅灰细线轮廓，左侧总转化线是否使用规范路径 | 短横线、粗曲线、装饰箭头、只画文字不画线 |
| 转化标签颜色 | 指标名使用辅助文本色，转化率数值使用主文本色，`changePp` 使用趋势 / 语义变量 | 所有数值都用蓝色、正负变化色与语义相反 |
| 数据标签描边 | 白色或浅色图内数据标签是否使用 `strokeAlign = OUTSIDE` 或等效外轮廓 | 使用居中描边导致文字发糊，或没有描边导致跨出图形时不可读 |
...
当优化对象为漏斗图时，`Optimization Plan` 必须显式列出：

```txt
funnel_token_strategy:
  item_fill:
  conversion_bar:
  conversion_line:
  conversion_label:
  ...
  change_pp:
  data_label_stroke:
funnel_geometry_strategy:
  variant:
  right_conversion_line_path:
  left_total_line_path:
  label_anchor:
```

不得只写“颜色调整为规范蓝色”或“转化线按规范优化”。必须说明依赖的全局变量族、当前模式解析值、以及转化线的几何路径或基准尺寸。
...
- 是否绑定主题变量并写出当前模式解析值？
- 如果是漏斗图，是否检查了 `funnelPalette/*`、全局文本 / 边框 / 趋势变量依赖、转化线基准路径和数据标签外描边？
- 是否对生成结果做了二次自查？
