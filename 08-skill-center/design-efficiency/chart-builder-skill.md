...
dimensionAxisInnerPadding = halfBand by default when x/category/time is a dimension axis
axisTickLength = 4px when shown
axisLabelLineHeight = 18px
plotArea = excludes axis label rail and x-axis label band
```

设计工具生成时，应先创建 `yAxisLabelRail`、`plotArea`、`xAxisLabelBand` 三个可编辑区域，双轴图还必须创建独立 `rightYAxisLabelRail`，再将网格线、轴线和数据图形约束在 `plotArea` 内。不得把 X 轴标签的 `10px` 偏移误用为 Y 轴标签的整体距离；左侧 Y 轴标签约束的是“标签右边缘距离绘图区左边界 `10px`”，文本右对齐；右侧异轴标签约束的是“标签左边缘距离绘图区右边界 `10px`”，文本左对齐；X 轴标签约束的是“标签文本框顶部距离底部轴线 `10px`”，文本框顶对齐。

维度轴上的数据图形必须有首尾留白策略。生成柱状图、折线图、面积图、柱线混合图等以 X 轴为维度轴的图表时，应先定义 `dimensionScaleRange`，默认让首个和最后一个数据位置分别从 `plotArea.left/right` 内缩半个 band，或使用 `plotWidth / dataCount / 2` 的等效留白。不得让首尾柱体、折线点、刻度或标签直接顶到坐标轴两侧边缘，除非业务明确要求边界对齐。

`series` 必须描述每个数据系列的可维护图层策略。折线 / 曲线 / 趋势线在 Figma / Relay / Zero 可编辑稿中应优先使用一个完整的 `VectorNode` / path-like 节点承载整条 series，节点名例如 `series/line/conversion_rate/path`。不得把同一条折线拆成多条独立 Line 节点来模拟连续路径；只有 hover 指示线、目标线、阈值线等非 series 辅助元素可使用单独 Line 节点。

柱状图 series 默认使用对应 `分类色板/item-*` 的实色填充，普通默认态不添加透明度。只有禁用、未选中、hover 弱化、背景参考柱、目标/实际嵌套等明确状态才允许降低 opacity。
...
笛卡尔坐标系图表的离散图例默认右上角水平对齐，图例整体右边缘与图表内容区右侧对齐；系列项之间默认 `gap = 20px`，系列项内部 `shape-label gap = 5px`。图例中只允许出现数据系列对应项，不得加入“左轴金额 / 右轴转化率”“柱线混合图 / 双轴”等轴说明、图表类型说明或其他非系列说明文本。
...
- `axis_layout_constraints` 必须写明 `yAxisLabelRailWidth = 50px`、`yAxisLabelRightPadding = 10px`、`xAxisLabelBandHeight >= 28px`、`xAxisLabelTopOffset = 10px`、`axisTickLength = 4px`。可编辑设计稿应创建独立 `yAxisLabelRail`、`plotArea`、`xAxisLabelBand`；双轴图还必须创建 `rightYAxisLabelRail`，并写明 `rightYAxisLabelLeftPadding = 10px`。网格线和数据图形只能落在 `plotArea` 内，左侧 Y 轴标签右对齐到 `plotArea.left - 10px`，右侧异轴标签左对齐到 `plotArea.right + 10px`，X 轴标签顶部为 `plotArea.bottom + 10px`。当 X 轴是维度轴时，还必须写明 `dimensionAxisInnerPadding`，默认首尾内缩半个 band 或等效留白。
- `series` 必须写明折线系列的节点合并策略。折线图、面积图边界线、柱线混合图中的折线 series 应使用单个完整 path / vector 表达一条 series；不得用多条独立 Line 节点拼接同一条折线，避免后续选择、调色、动效和组件替换时失去 series 语义。
...
- `interaction_policy` 必须按图表类型写明 axisPointer / 坐标轴指示器形态。折线图默认 hover 显示单根垂直 `1px` 实线 axisPointer、当前点和规范化 Tooltip；柱状图、柱线混合图默认 hover 只显示完整类目占位宽度的高亮矩形，不显示垂直 line 指示器。高亮矩形填充为 `rgba(0,0,0,0.03)`，宽度等于当前 `categoryBandWidth`，高度等于 `plotAreaHeight`，位置以当前类目中心对齐。不得使用虚线 hover 指示线、引擎默认横向 crosshair 或字段名 Tooltip 直接交付；hover layer 必须与实际绘图区 / chartHost 绑定；hover 点必须来自引擎坐标转换、实际渲染 geometry，或与 series 同层同 scale 绘制，避免指示器溢出和 MarkPoint 偏移。
...
- Y 轴标签是否与对应轴线 / 分隔线垂直居中对齐，端点标签也未被上移或下移。
- 双轴图右侧异轴标签是否左对齐，且标签左边缘距离绘图区右边界 `10px`。
- X 轴标签是否在底部轴线下方 `10px` 起排，且文本框行高为 `18px`。
- X 轴标签文本框是否顶对齐到 `plotArea.bottom + 10px`，而不是按文字中心或底边对齐。
- 维度轴是否有首尾留白，首尾数据点、柱体、刻度和标签没有贴住 `plotArea` 左右边缘。
- 网格线是否只在 `plotArea` 内绘制，没有伸入轴标签轨道或标题区。
- X 轴标签是否按组件策略抽样。
- 图例 shape 是否按图形语义选择：折线 `line 10×2`、面/柱/饼 `rectangle 10×10 r2`、点图 `dot 10×10`。
- 笛卡尔坐标系图例是否右上角对齐，系列间距是否为 `20px`，且没有非系列说明文本。
- 折线 series 是否为单个完整 path / vector 节点，而不是多条独立 Line 节点拼接。
- 柱状图默认柱体是否使用分类色板实色填充，未无故添加透明度。
...
- 折线图的坐标轴指示器是否为单根垂直 `1px` 实线。
- 柱状图 / 柱线混合图是否只显示 `rgba(0,0,0,0.03)` 高亮矩形，且矩形宽度为完整类目占位宽度、高度为 `plotAreaHeight`，没有额外垂直 line 指示器。
